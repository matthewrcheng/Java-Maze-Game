...
public class MazeBuilderBoruvka extends MazeBuilder implements Runnable {
	...
	protected void generatePathways() {
		...
		while (!completed) {
			...
			ArrayList<ArrayList<Point>> components = new ArrayList<ArrayList<Point>>(this.width*this.height);
			...
			for (Wallboard candidate : this.edge_candidates) {
				Wallboard edge = null;
				Point pt1 = new Point(candidate.getX(),candidate.getY());
				Point pt2 = getOtherCell(candidate);
				ArrayList<Point> pt1_comp = null;
				ArrayList<Point> pt2_comp = null;
				...
				for (ArrayList<Point> comp : components) {
					if (comp.contains(pt1)) {
						pt1_comp = comp;
					}
					if (comp.contains(pt2)) {
						pt2_comp = comp;
					}
				}
	...
	protected void assign_components(ArrayList<ArrayList<Point>> components) {
		...
		for (Wallboard edge : this.edges) {
			int x = edge.getX();
			int y = edge.getY();
			Point cur_pt = new Point(x,y);
			Point alt_pt = getOtherCell(edge);
			// we begin with the component belonging to the vertex equaling null
			ArrayList<Point> cur_comp = null;
			ArrayList<Point> alt_comp = null;
			...
			for (ArrayList<Point> comp : components) {
				...
				if (comp.contains(cur_pt)) {
					cur_comp = comp;
				}
				if (comp.contains(alt_pt)) {
					alt_comp = comp;
				}
			}
			...
			if (cur_comp == null & alt_comp == null) {
				ArrayList<Point> new_comp = new ArrayList<Point>(2);
				new_comp.add(cur_pt);
				new_comp.add(alt_pt);
		...
		if (first) {
			for (int[] vertex : this.vertices) {
				Point point = new Point(vertex[0],vertex[1]);
				ArrayList<Point> single_comp = new ArrayList<Point>(1);
	...
	protected Point getOtherCell(Wallboard wb) {
	...
	protected Wallboard getOtherWallboard(Wallboard wb) {
		...
		return new Wallboard(cell.x,cell.y,cd);
	}
