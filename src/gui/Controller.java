...
public class Controller {
    ...
    MazePanel panel;
    /**
     * This was added to allow for different levels each run.
     */
    Random rand;
    /**
    ...
    // whether a Wizard is being used or not
    boolean wizardDriver;
    // whether a WallFollower is being used or not
    boolean wfDriver;
    ...
    public void switchFromTitleToGenerating(int skillLevel) {
        ...
        if (!deterministic) {
        	currentState.setSeed(rand.nextInt());
        	currentState.start(this, panel);
    ...
    public void switchFromGeneratingToPlaying(Maze config) {
        ...
        if (wizardDriver == true) {
        	if (!reliableForward | !reliableLeft | !reliableRight | !reliableBackward) {
        		setRobotAndDriver(new UnreliableRobot(this), new Wizard());
        	}
        	else {
        		setRobotAndDriver(new ReliableRobot(this), new Wizard()); 
        	}
        	driver.setRobot(robot);
        	driver.setMaze(config);
        	if (driver != null && test == false) {
        		try {
        			if (driver.drive2Exit()) {
        				switchFromPlayingToWinning(getRobot().getOdometerReading());
        			}
        		} catch (Exception e) {
        			// TODO Auto-generated catch block
        			e.printStackTrace();
        			switchFromPlayingToWinning(-2);
        		}
        	}
        }
        if (wfDriver == true) {
        	if (!reliableForward | !reliableLeft | !reliableRight | !reliableBackward) {
        		setRobotAndDriver(new UnreliableRobot(this), new WallFollower());
        	}
        	else {
        		setRobotAndDriver(new ReliableRobot(this), new WallFollower()); 
        	}
        	driver.setRobot(robot);
        	driver.setMaze(config);
        	if (driver != null && test == false) {
    ...
    private boolean reliableForward;
	private boolean reliableLeft;
	private boolean reliableRight;
	private boolean reliableBackward;
	
	/**
	 * Returns whether or not the robot has a reliable forward sensor.
	 * @return boolean
	 */
	...
	/**
	 * Sets the robot's forward sensor to be reliable or not.
	 * @param reliable indication of whether the forward sensor is reliable
	 */
	protected void setReliableForward(boolean reliable) {
		reliableForward = reliable;
	}
	
	/**
	 * Returns whether or not the robot has a reliable left sensor.
	 * @return boolean
	 */
	...
	/**
	 * Sets the robot's left sensor to be reliable or not.
	 * @param reliable indication of whether the left sensor is reliable
	 */
	protected void setReliableLeft(boolean reliable) {
		reliableLeft = reliable;
	}
	
	/**
	 * Returns whether or not the robot has a reliable right sensor.
	 * @return boolean
	 */
	...
	/**
	 * Sets the robot's right sensor to be reliable or not.
	 * @param reliable indication of whether the right sensor is reliable
	 */
	protected void setReliableRight(boolean reliable) {
		reliableRight = reliable;
	}
	
	/**
	 * Returns whether or not the robot has a reliable backward sensor.
	 * @return boolean
	 */
	...
	/**
	 * Sets the robot's backward sensor to be reliable or not.
	 * @param reliable indication of whether the backward sensor is reliable
	 */
	protected void setReliableBackward(boolean reliable) {
		reliableBackward = reliable;
	}
