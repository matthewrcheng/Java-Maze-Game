...
public class StateWinning extends DefaultState {
    ...
    public void start(Controller controller, MazePanel panel) {
        ...
        if (controller.wizardDriver | controller.wfDriver) {
        	float battery = controller.getRobot().getBatteryLevel();
        	view.redrawFinish(panel, pathLength, battery);
        } 
        else {
        	view.redrawFinish(panel,-1,-1);
        }
